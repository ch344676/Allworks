...
describe("Testing the third route", function(err){
  ...
  describe("Testing the movie route", function(err){
  it("should handle and send the movie info", function(done){
    url
        .get("/mong/moviesurl/movieAll")
        .expect(200)
		.expect('Content-Type', /json/)
        .end(function(err,res){
          should.not.exist(err);
  ...
  it("should handle and send the JSON data Part 1", function(done){
    url
        ...
        .end(function(err,res){
          should.not.exist(err);
          var myObj = JSON.parse(res.text);
          myObj.name.should.be.equal("movie");
          done();
        });
